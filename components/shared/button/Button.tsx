...
const buttonStyles = cva("rounded-lg font-semibold flex gap-2 h-full w-full  justify-center items-center", {
  ...
  export default function Button({ intent,size, text,disabled,onClick,className, ...props }: ButtonProps) {
 
    return (
      <button className={`${buttonStyles({ intent, size })} ${className}`} {...props} onClick={onClick} disabled={disabled}>
        {text}
      </button>
    );
  }